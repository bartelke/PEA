...
# funkcja zajmujaca sie obliczeniami dla danej glebokosci a nastepnie wywoluje sie rekurencyjnie az dotrze do liscia:
def rushToLeaf(data, bound, actualCost, deepthLevel, currWay, visitedNodes):
    global finalCost

    for i in range(N):
        # sprawdzenie czy dany wiezcholek nie byl jeszcze odwiedzony:
        if (visitedNodes[i] == False) and (data[currWay[deepthLevel-1]][i] != 0):
            boundCopy = bound
            # dodanie kosztu dojscia do poprzedniego wierzcholka:
